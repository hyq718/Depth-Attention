...
class ModelArguments(QuantizationArguments, ProcessorArguments, ExportArguments, VllmArguments):
    ...
    tokenizer_name_or_path: Optional[str] = field(
        default=None,
        metadata={"help": "Optional tokenizer path or Hub id. Defaults to `model_name_or_path` when unset."},
    )
    ...
    depth_attention_recent_window: Optional[int] = field(
        default=None,
        metadata={"help": "Always include this many most-recent previous layers. Leave unset to use the config."},
    )
    attnres_block_size: Optional[int] = field(
        default=None,
        metadata={"help": "Block size for the block AttnRes baseline. Leave unset to use the config."},
    )
    attnres_recency_bias_init: Optional[float] = field(
        default=None,
        metadata={"help": "Initial recency bias for the block AttnRes aggregator. Leave unset to use the config."},
    )
    denseformer_dwa_dilation: Optional[int] = field(
        default=None,
        metadata={"help": "DenseFormer/DenseTransformer depth-weighted averaging dilation. Leave unset to use config."},
    )
    denseformer_dwa_period: Optional[int] = field(
        default=None,
        metadata={"help": "DenseFormer/DenseTransformer depth-weighted averaging period. Leave unset to use config."},
    )
    residual_baseline_num_streams: Optional[int] = field(
        default=None,
        metadata={"help": "Number of residual streams for the mHC baseline. Leave unset to use the config."},
    )
    interpolation_use_topk: bool = field(
        default=False,
        metadata={"help": "Use top-k softmax over the vocabulary when computing interpolated embeddings."},
    )
    ...
    num_latent_thoughts: int = field(
        default=0,
        metadata={
            "help": (
                "Reserved for compatibility with the LLaMA-Factory argument schema. "
                "Depth-Attention does not use latent-thought iterations."
            )
        },
    )
    ...
    train_from_scratch: bool = field(
        default=False,
        metadata={"help": "Randomly initialize the model from a config rather than loading pretrained weights."},
    )
