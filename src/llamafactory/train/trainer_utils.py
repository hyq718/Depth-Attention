...
def _create_ellisadam_optimizer(
    model: "PreTrainedModel",
    training_args: "Seq2SeqTrainingArguments",
    finetuning_args: "FinetuningArguments",
) -> "torch.optim.Optimizer":

    # 尝试从recpre导入ELLISAdam
    from ..extras.optim import ELLISAdam, get_param_groups    
    # decay_params, nodecay_params = [], []
    # decay_param_names = _get_decay_parameter_names(model)
    ...
    optim_kwargs = {
        ...
        "eps": training_args.adam_epsilon,
        "weight_decay": training_args.weight_decay,
        "update_clipping": finetuning_args.update_clipping,
        "atan_adam": finetuning_args.atan_adam,
        "running_init": finetuning_args.running_init,
        "tensor_wise_finite_check": finetuning_args.tensor_wise_finite_check,
        "tensor_wise_gradient_normalization": finetuning_args.tensor_wise_gradient_normalization,
        "decouple_wd": finetuning_args.decouple_wd,
        # "buffer_dtype": finetuning_args.buffer_dtype,
        # "eps_adjustment": finetuning_args.eps_adjustment,
        # "kahan_sum_compensation": finetuning_args.kahan_sum_compensation,
        # "adafactor_like_beta_corrections": finetuning_args.adafactor_like_beta_corrections,
    }
    
    optimizer = ELLISAdam(param_groups, **optim_kwargs)
    logger.info_rank0("Using ELLISAdam optimizer.")
    return optimizer
...
def create_custom_optimizer(
    ...
    if finetuning_args.use_ellisadam:
        return _create_ellisadam_optimizer(model, training_args, finetuning_args)
