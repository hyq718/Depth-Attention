...
def _arg_or_config(model_args: "ModelArguments", config: "PretrainedConfig", arg_name: str, *config_names: str, default=None):
...
def load_tokenizer(model_args: "ModelArguments") -> "TokenizerModule":
    ...
    try:
        processor = AutoProcessor.from_pretrained(tokenizer_name_or_path, **init_kwargs)
...
def load_model(
    ...
    """
    if model_args.patch_method is not None and model_args.patch_method != "vanilla":
        if model_args.patch_method not in _PATCH_METHODS:
            raise ValueError(
                f"Unknown patch_method={model_args.patch_method!r}. "
                f"Choose one of: {sorted(_PATCH_METHODS) + ['vanilla']}."
            )
        _PATCH_METHODS[model_args.patch_method]()

    init_kwargs = _get_init_kwargs(model_args)
    config = load_config(model_args)

    # ---- Depth-Attention / baseline hyperparameters (see README) ----
    ...
    if model_args.use_qk_norm is not None:
    ...
    if model_args.patch_method == "depth_attention":
        ...
        config.attnres_block_size = _arg_or_config(
            model_args, config, "attnres_block_size", "attnres_block_size", default=12
        )
        config.attnres_recency_bias_init = _arg_or_config(
        ...
        denseformer_dilation = _arg_or_config(
            model_args,
            config,
            "denseformer_dwa_dilation",
            "denseformer_dwa_dilation",
            "densetransformer_dwa_dilation",
            default=1,
        )
        denseformer_period = _arg_or_config(
            model_args,
            config,
            "denseformer_dwa_period",
            "denseformer_dwa_period",
            "densetransformer_dwa_period",
            default=1,
        )
        config.denseformer_dwa_dilation = denseformer_dilation
        config.densetransformer_dwa_dilation = denseformer_dilation
        ...
        num_streams = _arg_or_config(
            model_args, config, "residual_baseline_num_streams", "residual_baseline_num_streams", "mhc_num_streams", default=4
        )
