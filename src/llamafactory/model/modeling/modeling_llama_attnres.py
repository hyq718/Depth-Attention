...
class BlockAttentionResidualAggregator(nn.Module):
    """Block AttnRes depth-attention aggregator (aligned with wdlctc/open-attention-residuals).

    Differs from the original `paper-line-871 zero-init uniform softmax` design by adding a
    learnable scalar `recency_bias` that is added to the partial_block's logit before softmax.
    With `recency_bias_init=3.0`, the partial_block (=current cumulative residual) gets ~95%
    softmax weight at init, so the model is mathematically close to standard PreNorm Llama
    at step 0 and training is stable from the start. As training progresses, the bias and
    pseudo_query co-adapt to learn meaningful cross-depth attention.
    """
    def __init__(
        self,
        hidden_size: int,
        ...
        self.recency_bias_init = float(recency_bias_init)
        self.pseudo_query = nn.Parameter(torch.zeros(hidden_size))
        self.norm = LlamaRMSNorm(hidden_size, eps=eps)
        self.recency_bias = nn.Parameter(torch.tensor(self.recency_bias_init))

    def reset_parameters(self):
        with torch.no_grad():
            self.pseudo_query.zero_()
            self.norm.weight.fill_(1.0)
            self.recency_bias.fill_(self.recency_bias_init)
    ...
    def forward(
        ...
        if partial_block is not None:
            ...
            partial_logits = (
                torch.matmul(partial_block, query).float() * partial_inv_rms
            )
            # Recency bias: large positive scalar added to partial_block's logit so that
            # softmax puts most of its mass on partial_block at init (model ≈ vanilla).
            partial_logits = partial_logits + self.recency_bias.float()
            logits = torch.cat([completed_logits, partial_logits.unsqueeze(0)], dim=0)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        if self.residual_baseline in ("hyper_connection", "mhc"):
            ...
            _bias_init = float(getattr(config, "attnres_recency_bias_init", 3.0))
            self.attnres_attn_aggregator = BlockAttentionResidualAggregator(
                config.hidden_size,
                config.rms_norm_eps,
                recency_bias_init=_bias_init,
    ...
    def forward_attnres(
        ...
        completed_stack: torch.Tensor,
        completed_inv_rms: torch.Tensor,
        partial_block: torch.Tensor,  # cumulative residual stream (never None after init)
        partial_sublayers: int,
        ...
        collect_layer_kv: bool = False,
        **kwargs,
    ) -> Tuple[torch.FloatTensor, List[torch.Tensor], torch.Tensor, torch.Tensor, torch.Tensor, int]:
        # === ATTN sublayer ===
        # AttnRes mix over completed_blocks + cumulative partial_block.
        ...
        # Cumulative add — partial_block is the FULL residual stream, never reset.
        partial_block = partial_block + hidden_update
        partial_sublayers += 1
        ...
        # === Block boundary commit (at END of layer, like reference repo) ===
        # When this layer is the last of its block, append a SNAPSHOT of the cumulative
        # partial_block to completed_blocks. partial_block itself keeps accumulating.
        if partial_sublayers >= attnres_block_size:
            completed_blocks = completed_blocks + [partial_block]
            completed_stack = torch.cat([completed_stack, partial_block.unsqueeze(0)], dim=0)
            partial_inv_rms = BlockAttentionResidualAggregator.compute_inv_rms(
                partial_block, self.attnres_mlp_aggregator.norm.variance_epsilon
            )
            completed_inv_rms = torch.cat(
                [completed_inv_rms, partial_inv_rms.unsqueeze(0)], dim=0
            )
            partial_sublayers = 0
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # Cumulative residual stream (reference repo semantics). partial_block starts
        # equal to the token embedding and grows across all layers; it is NEVER reset.
        # At block boundaries, a snapshot of partial_block is appended to completed_blocks.
        attnres_partial_block = hidden_states.clone() if use_attnres_baseline else None
        ...
        if use_stream_baseline:
            ...
            # Reference repo emits norm(partial_block) directly at the end (no extra final
            # aggregation). We keep an explicit final_aggregator for symmetry; with
            # recency_bias_init=3.0 it returns ~95% partial_block at init (≈ vanilla path)
            # and learns to optionally mix earlier snapshots over training. Block boundaries
            # are committed inside forward_attnres at end of boundary layers, so no extra
